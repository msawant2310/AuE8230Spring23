...
class TurtleBot:
    ...
    def __init__(self):
        ...
        rospy.init_node('cirle', anonymous=True)
    ...
    def circle(self):
        ...
        linear_velocity = rospy.get_param('~l_vel')
        angular_velocity = rospy.get_param('~a_vel')
        self.vel.linear.x = linear_velocity
        self.vel.angular.z = angular_velocity

        timestamp = rospy.Time.now().to_sec()

        while rospy.Time.now().to_sec() - timestamp < 20:
            self.velocity_publisher.publish(self.vel)
            self.rate.sleep()
        self.vel.linear.x = 0
        self.vel.angular.x = 0
